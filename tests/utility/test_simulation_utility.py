...
def test_apply_phase_to_qubit_zero_state_unaffected():
    ...
    # Arrange
    psi = np.array([1.0 + 0j, 0.0, 0.0, 0.0])
    expected = psi.copy()
...
def test_apply_phase_to_qubit_two_qubits_targets_correct_qubit():
    ...
    expected = np.array(
        [0.5, 0.5, 0.5 * np.exp(1j * phase), 0.5 * np.exp(1j * phase)]
    )
    assert np.allclose(result, expected), f"Expected {expected} but got {result}."
    ...
    expected = np.array(
        [0.5, 0.5 * np.exp(1j * phase), 0.5, 0.5 * np.exp(1j * phase)]
    )
    assert np.allclose(result, expected), f"Expected {expected} but got {result}."
...
def test_apply_phase_corrections_matches_manual_loop():
    ...
    psi = rng.random(8) + 1j * rng.random(8)
    psi /= np.linalg.norm(psi)
    ...
    for qubit in range(n):
        if phases[qubit] != 0:
            expected = apply_phase_to_qubit(
                expected, qubit, dim, n, phase=phases[qubit]
            )
...
def test_post_process_split_mean():
    ...
    assert all(
        abs(mean_array[i] - mean_expected[i]) < 1e-9
...
def test_permute_brings_qubits_to_front():
    ...
    _, perm = permute_to_adjacent(psi, q_ctr_v=0, q_trg_v=3, n=n)
...
def test_permute_consecutive_qubits_unchanged():
    ...
    psi_permuted, _ = permute_to_adjacent(psi, q_ctr_v=0, q_trg_v=1, n=n)
...
def test_permute_preserves_norm():
    ...
    psi_permuted, _ = permute_to_adjacent(psi, q_ctr_v=1, q_trg_v=4, n=n)
...
def test_permute_preserves_all_amplitudes():
    ...
    psi_permuted, _ = permute_to_adjacent(psi, q_ctr_v=0, q_trg_v=3, n=n)
