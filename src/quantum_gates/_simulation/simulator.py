...
class MrAndersonSimulator(object):
    ...
    def run(self,
        ...
        for shot in all_results:
            ...
            barrier_statevectors.append(shot["barrier_statevectors"])
    ...
    def _preprocess_circuit(self, t_qiskit_circ, n_qubit_used: int, used_logicals) -> tuple:
        ...
        for i, op in enumerate(raw_data):
            ...
            # ---- Not yet implemented fancy gates ----
                ...
                raise NotImplementedError("Use barrier with label 'save' for statevector saving; direct statevector_readout not implemented yet.")
...
def _single_shot(args: dict) -> np.array:
    ...
    barrier_statevectors = []  # Store barrier statevectors if needed
        ...
        if flag == 0:       
            ...
            if isinstance(d, tuple) and d[0] == "mid_measurement":
                ...
                if d.label is not None and 'save' in d.label:
                    ...
                    psi_corrected = circ.statevector_readout(psi)
                    barrier_statevectors.append((d.label, psi_corrected))
            
            else:
                op_name = d.operation.name
                if op_name == "statevector_readout":
                    raise NotImplementedError("To save statevector, use barrier with label 'save' instead.")
